...
resource "aws_subnet" "eks_private_subnets" {
  ...
  tags = merge(
    var.common_tags,
    {
      Name = "eks-private-${var.clusters_name}-${data.aws_availability_zones.availability_zones.names[count.index]}"
    },
  )
  lifecycle {
    ignore_changes = [
...
resource "aws_subnet" "eks_public_subnets" {
  count             = length(var.eks_public_subnets_list)
  cidr_block        = element(var.eks_public_subnets_list, count.index)
  vpc_id            = aws_vpc.eks_vpc.id
  availability_zone = data.aws_availability_zones.availability_zones.names[count.index]
  ...
  tags = merge(
    var.common_tags,
    {
      "Name" = "eks-public-${var.clusters_name}-${data.aws_availability_zones.availability_zones.names[count.index]}"
    },
  )
